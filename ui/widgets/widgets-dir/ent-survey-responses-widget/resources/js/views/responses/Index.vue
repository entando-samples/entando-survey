<template>
  <div>
            ...
            <tr v-for="(survey, index) in surveys.data" :key="survey.id" :class="index % 2 === 0 ? 'bg-gray-100' : ''">
              ...
              <!-- <td class="px-2 py-3">
                <div class="font-medium" style="max-width: 150px">
                  {{ (survey.pathologies || []).map((item) => item.title).join(', ') }}
                </div>
              </td> -->
              <td class="px-2 py-3 flex justify-center">
                <div class="flex items-center space-x-2">
                  <button
                    ...
                    class="px-5 font-semibold text-white bg-primary rounded p-1 border-2 border-primary"
                  >
                    Schedule
                  </button>
                  ...
                  <router-link
                    ...
                    class="p-1 border-2"
                    :class="
                      survey.can_be_edited ? 'border-primary rounded bg-primary' : 'border-gray-300 rounded bg-gray-400'
                    "
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                  ...
                  >
                    <span
                      ...
                      :class="
                        survey.patients_count > 0 ? 'hover:text-green-900 hover:font-extrabold' : 'cursor-not-allowed'
                      "
    ...
    <b-modal size="3xl" title="schedula il questionario" v-if="modals.scheduler" @close="modals.scheduler = false">
      <p class="font-bold text-gray-600">Nome Questionario</p>
      <p class="mt-1 text-lg">Questionario post visita di centratura</p>

      <div class="mt-8">
        <label class="block font-semibold text-gray-600">Quando vuoi schedularlo?</label>
        <div class="mt-1 flex justify-between">
          ...
          <label class="flex items-center space-x-3">
            <input type="number" min="0" class="h-5 w-14 border border-primary py-3" />
      ...
      <div class="mt-8">
        <label class="block font-semibold text-gray-600">Scegli quando ripetere</label>
        <div class="mt-1 flex justify-between">
          <label class="flex items-center space-x-3">
            <span>Ripeti</span>
            <select class="p-1 border border-primary">
      ...
      <template #footer>
        <div class="flex gap-x-4 flex-row-reverse">
          ...
          <b-button variant="primary-alt" @click="modals.scheduler = false">Indietro</b-button>
        </div>
      </template>
    </b-modal>
    ...
    >
      <h3>
        ...
        <span class="font-bold">{{ deleteSurveyModal.remind.survey.title }}</span
        >.
      ...
      <template #footer>
        <div class="flex gap-x-4 flex-row-reverse">
          <b-button @click="deleteItem($event, deleteSurveyModal.remind.survey.id)">Confirm</b-button>
          <b-button variant="primary-alt" @click="deleteSurveyModal.remind.show = false">Indietro</b-button>
        </div>
      </template>
    </b-modal>
