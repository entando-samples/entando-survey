    ...
    <div class="bg-white p-3 rounded-md">
        <table class="w-full">
            ...
            <tr v-else class="border-t border-b" v-for="message in messages.data" :key="message.id">
                ...
                <td class="py-4 px-3 text-sm" style="max-width: 200px;">{{ message.body | strlimit(30) }}</td>
                <td class="py-4 px-3 text-sm text-gray-500">Ricevuto il: {{ message.created_at | date }}</td>
                <td class="py-4 px-3">
                    <router-link
